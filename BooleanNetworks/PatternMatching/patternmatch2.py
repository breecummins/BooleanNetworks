        ...
        for p,P in patterns:
            for n in getNextNodes(startnode,pDict['outedges']):  # every wall has an outgoing edge by graph construction
                if len(match) == 1 or set(previouspattern).intersection(mwl.pathDependentStringConstruction2(match[-2],match[-1],n,pDict['walldomains'],pDict['varsaffectedatwall'][match[-1]])): # consistency check to catch false positives
                    walllabels = [w for q in getNextNodes(n,pDict['outedges']) for w in mwl.pathDependentStringConstruction2(match[-1],n,q,pDict['walldomains'],pDict['varsaffectedatwall'][n]) ]
