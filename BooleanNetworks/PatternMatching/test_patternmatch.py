...
def testme(showme=1):
    ...
    match = matchCyclicPattern(pattern,inds,paramDict,showfirstwall=0)
    ...
    match = matchCyclicPattern(patterns[0],inds,paramDict,showfirstwall=0)
    if showme: print match==[(0,4,9,10,6,3,0)]
    match = matchCyclicPattern(patterns[1],inds,paramDict,showfirstwall=0)
    if showme: print 'None' in match
    ...
    for p,s in zip(patterns,solutions):
        match = matchCyclicPattern(p,inds,paramDict,showfirstwall=0)
        if s:
