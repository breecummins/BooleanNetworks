...
def onewayforcing():
    call(["dsgrn network "+os.path.expanduser("~/GIT/DSGRN/networks/6D_OneWayForcing.txt")+" analyze morseset 4 1213 >dsgrn_output.json"],shell=True)
...
def twowayforcing():
    call(["dsgrn network "+os.path.expanduser("~/GIT/DSGRN/networks/6D_TwoWayForcing.txt")+" analyze morseset 4 28015 >dsgrn_output.json"],shell=True)
    f=open('patterns.txt','w')
    f.write('X1 max, X2 max, Y1 min, Y2 min, Z1 max, Z2 max, X1 min, X2 min, Y1 max, Y2 max, Z1 min, Z2 min\nX1 max, Y1 min, Z1 max, X1 min, Y1 max, Z1 min\nX2 max, Y2 min, Z2 max, X2 min, Y2 max, Z2 min')
    f.close()
...
def search(forcing=1):
    if forcing==1:
        ...
        return None
    pm.call_PatternMatch_ShaunFormat_WriteFile()
